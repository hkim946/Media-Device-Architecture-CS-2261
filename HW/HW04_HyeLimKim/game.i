...
enum {BLACKID = (256-5), GREENID, REDID, WHITEID, GRAYID};
...
void initGame() {
 ...
 initRocket();
    ...
    aliensRemaining = 15 - 1;
    score = 0;


    unsigned short colors[5] = {0, ((0) | (31)<<5 | (0)<<10), ((31) | (0)<<5 | (0)<<10), ((31) | (31)<<5 | (31)<<10), ((15) | (15)<<5 | (15)<<10)};


    DMANow(3, alienPal, ((unsigned short *)0x5000000), 512/2);
...
void updateRocket() {
 ...
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5))) && rocket.col >= rocket.cdel) {
  rocket.col -= rocket.cdel;
        rocket.cDirection = -1;
 } else if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && rocket.col + rocket.width - 1 < 240 - rocket.cdel) {
        rocket.col += rocket.cdel;
        rocket.cDirection = 1;
 } else {
        rocket.cDirection = 0;
 ...
 if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0)))) && rocket.bulletTimer >= 15) {
  fireBullet();
...
void updateAlien(ALIEN* a) {
 if (a->active) {
  ...
  if (a->row <= 1 || a->row + a->height-1 >= 119)
   a->rdel *= -1;
  if (a->col <= 1 || a->col + a->width-1 >= 240 -2)
   a->cdel *= -1;
